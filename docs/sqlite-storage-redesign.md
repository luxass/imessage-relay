...
| Read update with advancing timestamp | `(date_read, ROWID)` stream | `date_read` | Targeted nonterminal recheck, then reconciliation | Filesystem debounce or fallback check | Equal timestamps and old-row read update |
...
| Delivery update with advancing timestamp | `(date_delivered, ROWID)` stream if validated | `date_delivered` | Targeted nonterminal recheck, then reconciliation | Filesystem debounce or fallback check | Old-row delivery update |
